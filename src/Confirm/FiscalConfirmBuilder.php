...
/**
 * Подтверждение платежа с фискализацией.
 *
 ...
 *
 * @method $this setSubtotal(string|int $subtotal) Сумма оплаты.
 ...
 */
class FiscalConfirmBuilder extends BaseBuilder implements ContainerAwareInterface
{
    ...
    protected function registerParameters(): void
    {
        ...
        $this->parameters->add(
            new ScalarParameter(CanonicalParameterName::SHOP_ID, UnitellerParameterName::SHOPID, 64)
        );
        // Идентификатор заказа в системе Merchant, соответствующий данному платежу. Строка UTF-8 длиной до 64 символов.
        $this->parameters->add(
            new ScalarParameter(CanonicalParameterName::ORDER_ID, UnitellerParameterName::ORDER_ID, 64)
        );
        // Чек
        $this->parameters->add(
            new ReceiptParameter(CanonicalParameterName::RECEIPT, UnitellerParameterName::RECEIPT)
        );
        // Уточненная сумма покупки в валюте, оговорённой в договоре с банком-эквайером.
        $this->parameters->add(
    ...
    protected function validateRequired(): void
    {
        ...
        if (!$this->hasShopId()) {
            throw new RequiredParameterException(UnitellerParameterName::SHOPID);
        }
        if (!$this->hasOrderId()) {
            throw new RequiredParameterException(UnitellerParameterName::ORDER_ID);
        }
        if (!$this->hasReceipt()) {
    ...
    {
        $arr = [
            UnitellerParameterName::SHOPID   => $this->getShopId(),
        ...
        return $this->signatureCreator->setParameters($arr)->createMd5WithoutDelimiter();
    }
    ...
    {
        return ApiEndpoints::FISCAL_CONFIRM;
    }
    ...
    public function process()
    {
        ...
        if ($this->debug) {
            ...
            $this->logger->debug(
                'Receipt: ' . PHP_EOL . json_encode($this->getReceipt(), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE)
            );
        }
